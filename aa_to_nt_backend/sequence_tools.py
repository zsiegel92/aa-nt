...
def get_reverse_complement(nucleotides: str) -> str:
    return "".join(
        NUCLEOTIDE_COMPLEMENTS[nucleotide] for nucleotide in reversed(nucleotides)
    )
...
def get_longest_run(sequence: str) -> tuple[int, str, int]:
    ...
    longest = max(
        (
            substring
            for character in set(sequence)
            for substring in re.findall(f"{character}+", sequence)
        ),
        key=len,
    )
...
def build_nucleotide_list(
    ...
    *,
    ...
    for index, amino_acid in enumerate(amino_acids):
        if (
            wild_type
            and wild_type_amino_acids
            and amino_acid == wild_type_amino_acids[index]
        ):
...
def swap_codon_inplace(
    nucleotides: list[str], amino_acid_index: int, amino_acid: str | None
) -> None:
    resolved_amino_acid = (
        amino_acid or CODON_TO_AMINO_ACID[nucleotides[amino_acid_index].upper()]
    )
...
def fix_repeat_nucleotides(
    ...
    *,
    ...
    if (
        wild_type_amino_acids is not None
...
def translate_wild_type_to_amino_acids(
    wild_type: str, expected_length: int
) -> str | None:
    if not wild_type or len(wild_type) % 3 != 0:
        return None
    codons = (
        wild_type[index : index + 3].upper() for index in range(0, len(wild_type), 3)
    )
